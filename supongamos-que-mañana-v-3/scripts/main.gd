...
func terminar_juego(gano: bool):
	...
	# CONNECT_ONE_SHOT: la conexión se elimina automáticamente después de disparar una vez.
	# Esto evita el await anidado que dejaba game_finished = true para siempre.
	day_transition.transition_finished.connect(
		_on_transicion_terminada.bind(gano),
		CONNECT_ONE_SHOT
	)
	day_transition.mostrar(GameState.dia_actual, gano)
