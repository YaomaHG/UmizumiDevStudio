...
func _disenar_fase_almohadas():
	...
	# Zona verde óptima (Esponjosidad perfecta, 45%-55% = ancho 36px centrado)
	perfect_zone = ColorRect.new()
	perfect_zone.color = Color(0.15, 0.72, 0.35, 0.85) # Verde óptimo
	perfect_zone.size = Vector2(bar_width * 0.10, bar_height) # Encogido a 10%
	perfect_zone.position = Vector2(bar_width * 0.45, 0)
