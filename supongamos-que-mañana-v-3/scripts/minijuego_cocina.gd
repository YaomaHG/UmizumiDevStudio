...
func _disenar_fase_fuego():
	...
	# Perfect Temperature Zone (60% to 80% = 240px to 320px inside bar)
	var perfect_zone = ColorRect.new()
	perfect_zone.color = Color(0.15, 0.72, 0.35, 0.85) # Verde brillante óptimo
	perfect_zone.size = Vector2(bar_width * 0.20, bar_height)
	perfect_zone.position = Vector2(bar_width * 0.60, 0)
