...
func _ready():
	# Dificultad progresiva según el día
	var dia = GameState.dia_actual
	score_required = min(7, 3 + dia) # Día 1: 4, Día 2: 5, Día 3: 6, Día 4+: 7 basuras
	var vel_x = 100.0 + dia * 25.0
	var vel_y = 80.0 + dia * 20.0
	scorpion_velocity = Vector2(vel_x, vel_y)
